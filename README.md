# 의정부 경전철 월간 HTML 분석 보고서 자동 생성 시스템 (uijeongbu-lrt-report-skill)

본 프로젝트는 의정부 경전철의 월별 승하차 CSV 데이터를 기상 정보 및 지역 행사 정보와 연계 분석하여 프리미엄 품질의 HTML 보고서를 생성하는 시스템입니다.

## 1. 프로젝트 개요
의정부 경전철 이용 현황의 단순 통계 조회를 넘어 기상 요인(강수량, 기온, 강풍 등) 및 의정부시의 주요 문화/행사 일정과의 연관성을 분석합니다. 분석 결과를 시각화된 차트와 함께 직관적인 HTML 보고서로 제공하여 경전철 운영 및 마케팅 인사이트를 도출하도록 돕습니다.
...
필요한 외부 라이브러리를 설치합니다.
```bash
pip install pandas matplotlib jinja2 pyyaml beautifulsoup4 requests pyarrow
```

## 5. 실행 방법
특정 월의 승하차 현황 보고서를 수집부터 렌더링까지 일괄 실행합니다.
```bash
python scripts/run_monthly_report.py --period 2026-05
```

## 6. 월간 보고서 생성 예시
위 실행 명령어를 작동하면 `data/raw/ridership/`에서 `202605`에 매칭되는 데이터를 찾고, 날씨와 행사를 자동 수집 및 캐싱한 후, 종합적인 분석과 시각화 차트를 거쳐 `outputs/monthly/2026-05/report.html` 경로에 미려한 리포트 파일을 출력합니다.
